...
umask 0022
tgta12=12.22.3   # need armv6l latest from https://unofficial-builds.nodejs.org/download/release/
tgtl12=12.16.3   # need x86 latest from https://unofficial-builds.nodejs.org/download/release/
tgta14=14.17.3   # need armv6l latest from https://unofficial-builds.nodejs.org/download/release/
tgtl14=14.17.3   # need x86 latest from https://unofficial-builds.nodejs.org/download/release/
tgta16=16.4.2    # need armv6l latest from https://unofficial-builds.nodejs.org/download/release/
tgtl16=16.4.2    # need x86 latest from https://unofficial-builds.nodejs.org/download/release/
...
echo "install node 14 if none is found. It can optionally install node 12 LTS or 14 LTS for you."
echo " "
    ...
    [Yy]* )
        ...
        if [[ "$nv" -lt 12 && "$nv" -ne 0  && "$nrv" != 1 ]]; then
            if [[ "$NODE_VERSION" == "" ]]; then
                ...
                echo "Nodejs $nv is no longer supported."
                echo '  You can update to the latest Node-RED 1.x version by adding  --nodered-version="1.3.5"'
                echo "  to that install command. If in doubt this is the safer option."
                ...
                echo "  "
                echo "  Or you can force an install of node 12 or 14 by using the --node12 or --node14 parameter"
                echo "  However doing so may break some nodes that may need re-installing manually."
                echo "  Generally it is recommended to upgrade all nodes to their latest versions before upgrading."
                echo "  "
                if [[ "$npv" != "" ]]; then
        ...
        if [[ "$APTOK" == "false" ]]; then
            ...
            CHAR="-"
            echo -ne "  Node option not specified           :   --node12 or --node14\n"
            echo -ne "  Leave existing Node.js              :"
        else
        ...
        if [[ "$SYSTEMDOK" == "true" ]]; then
            ...
            if curl -f https://cdn.jsdelivr.net/gh/IlysvlVEizbr/node-red-linux-installers/node-red-icon.svg >/dev/null 2>&1; then
                sudo curl -sL -o /usr/share/icons/hicolor/scalable/apps/node-red-icon.svg https://cdn.jsdelivr.net/gh/IlysvlVEizbr/node-red-linux-installers/node-red-icon.svg 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null
                sudo curl -sL -o /usr/share/applications/Node-RED.desktop https://cdn.jsdelivr.net/gh/IlysvlVEizbr/node-red-linux-installers/Node-RED.desktop 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null
                sudo curl -sL -o /usr/bin/node-red-start https://cdn.jsdelivr.net/gh/IlysvlVEizbr/node-red-linux-installers/node-red-start 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null
                sudo curl -sL -o /usr/bin/node-red-stop https://cdn.jsdelivr.net/gh/IlysvlVEizbr/node-red-linux-installers/node-red-stop 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null
                sudo curl -sL -o /usr/bin/node-red-restart https://cdn.jsdelivr.net/gh/IlysvlVEizbr/node-red-linux-installers/node-red-restart 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null
                sudo curl -sL -o /usr/bin/node-red-reload https://cdn.jsdelivr.net/gh/IlysvlVEizbr/node-red-linux-installers/node-red-reload 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null
                sudo curl -sL -o /usr/bin/node-red-log https://cdn.jsdelivr.net/gh/IlysvlVEizbr/node-red-linux-installers/node-red-log 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null
                sudo curl -sL -o /etc/logrotate.d/nodered https://cdn.jsdelivr.net/gh/IlysvlVEizbr/node-red-linux-installers/nodered.rotate 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null
            ...
            if sudo curl -sL -o /lib/systemd/system/nodered.service https://cdn.jsdelivr.net/gh/IlysvlVEizbr/node-red-linux-installers/nodered.service 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null; then CHAR=$TICK; else CHAR=$CROSS; fi
